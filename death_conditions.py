...
def death_by_character(killer, death_message, hint):
...
def check_for_death_by_room(room):
    if room == 'basement' and 'lamp' not in MainChar.inventory:
        ...
        hint = 'Maybe a light source would have been useful.'
        death_by_character(MainChar.current_place.character, death_message, hint)
...
def check_for_death_by_item(room, items):
    if room == 'basement' and 'lamp' not in items:
        ...
        hint = 'Inretrospective, possibly not the smartest choice giving your lamp away in a dark cellar just like that.'
        death_by_character(MainChar.current_place.character, death_message, hint)