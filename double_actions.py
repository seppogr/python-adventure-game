...
def change_place(direction):
    ...
    print_text(f'You go from {old_place} into {new_place}.')
    ...
    if direction in MainChar.current_place.directions.keys():
        if MainChar.current_place.door['direction'] == direction and MainChar.current_place.door['open'] == False:
            if(MainChar.current_place.door['unlocked_by'] in MainChar.inventory):
                print_text(f'Your {MainChar.current_place.door['unlocked_by']} unlocks the door!')
                change_place(direction)
            else:
                print_text(f'The {direction} door is locked. It looks like you need a {MainChar.current_place.door['unlocked_by']} to proceed.')
        else:
            change_place(direction)
    else:
        print_text(f'There is no way to reach {direction} from here.')
...
# with ask command.
def chat(character):
    if character in MainChar.current_place.character:
        print_text(f'You greet the {character}. He says "{npc_data[character]['greeting']}".')
        print_text(f'The {character} chats with you. Some topics pique your interest: ')
        topics = npc_topics(npc_conversation[character].keys())
        print_text(f'{print_in_colour(topics, BLUE)}')

        if len(npc_data[character]['items']) > 0:
            print_text(f'I also have these, if you need: ')
            npc_items = extract_list(npc_data[character]['items'])
            print_text(f'{print_in_colour(npc_items, GREEN)}')
    else:
        print_text(f'It seems {character} is not here. {MainChar.current_place.character.capitalize()} is amused when you talk by yourself.')
...
def ask(noun):
    if(noun in npc_conversation[MainChar.current_place.character].keys()):
        print_text(f'The {MainChar.current_place.character} says "{npc_conversation[MainChar.current_place.character][noun]}"')
    else:
        print_text(f'{noun.capitalize()} is something I know nothing about.')
...
def describe(noun):
    if noun in MainChar.current_place.character:
        print_text(npc_data[noun]['description'])
    elif noun in MainChar.current_place.items or noun in npc_data[MainChar.current_place.character]['items']:
        print_text(items[noun]['description'])
    else:
        print_text(f'You glance about the room but there is no {noun} here.')
    ...
    if item in MainChar.inventory:
        if item == npc_data[MainChar.current_place.character]['wants']:
            ...
            if(MainChar.remove_from_inventory(item)):
                print_text(f'{MainChar.current_place.character.capitalize()} thanks you heartily as you give your {item} away.')
        else:
            print_text(f'{MainChar.current_place.character.capitalize()} does not care for the {item}.')
    else:
        print_text(f'You rummage and rummage through your bag, but there is no {item} there. You cannot give away what you do not have.' )

def take(item):
    if item in MainChar.current_place.items:
        MainChar.add_to_inventory(item)
        MainChar.current_place.items.remove(item)
        print_text(f'You add {items[item]['synonym'].upper()} into your bag.')
    else:
        print_text(f'Hmmm. You take a long look around and it seems there is no {item} in the room.')

def request(item):
    if item == npc_data[MainChar.current_place.character]['wants']:
        print_text(f'{MainChar.current_place.character} explains: "No! Never in my life shall I part again from this precious {item}!"')
    elif item in npc_data[MainChar.current_place.character]['items']:
        MainChar.add_to_inventory(item)
        npc_data[MainChar.current_place.character]['items'].remove(item)
        print_text(f'You politely inform that the {MainChar.current_place.character}\'s {item} is required in your investigation. They grudgingly hand it over.')
    else:
        print_text(f'The {MainChar.current_place.character} hastily explains that the {item} is not in their possession.')
